...
export default function HumanizePage({ onBack }: HumanizePageProps) {
  const text = useAppStore((s) => s.humanizeText);
  const setText = useAppStore((s) => s.setHumanizeText);
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden inner-body" style={{ padding: '12px 12px 8px', WebkitOverflowScrolling: 'touch' }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {modes.map((m) => (
                <button key={m} onClick={() => setMode(m)}
                  className={`px-3 py-1.5 rounded-full text-[11px] font-semibold capitalize ${mode === m ? 'gradient-purple-btn text-primary-foreground shadow-card' : 'bg-card text-muted-foreground'}`}
            ...
            <div className="flex gap-1.5">
              {intensities.map((int) => (
                <button key={int} onClick={() => setIntensity(int)}
                  className={`px-3 py-1.5 rounded-full text-[11px] font-semibold capitalize ${intensity === int ? 'gradient-purple-btn text-primary-foreground shadow-card' : 'bg-card text-muted-foreground'}`}
        ...
        {loading && (
          <div className="mt-6 flex flex-col items-center py-8">
            ...
            <div className="mt-4 w-full"><div className="h-20 rounded-2xl animate-shimmer" /></div>
          </div>
        )}
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="space-y-3">
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-card rounded-xl p-3 text-center shadow-card" style={{ border: '1px solid hsl(var(--border))' }}>
                      ...
                      <span className="text-xl font-bold font-mono" style={{ color: scoreComparison.before >= 75 ? '#F87171' : scoreComparison.before >= 56 ? '#FBBF24' : '#34D399' }}>{scoreComparison.before}%</span>
                    </div>
                    ...
                    <div className="bg-card rounded-xl p-3 text-center shadow-card" style={{ border: '1px solid hsl(var(--border))' }}>
                      ...
                      <span className="text-xl font-bold font-mono" style={{ color: scoreComparison.after >= 75 ? '#F87171' : scoreComparison.after >= 56 ? '#FBBF24' : '#34D399' }}>{scoreComparison.after}%</span>
                    </div>
                  </div>
